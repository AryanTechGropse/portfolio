import Self from "../models/Self.modal.js";
...
export const addSelf = async (request, reply) => {
  try {
    const { title, profilePhoto, description, status } = request.body;
    const newSelf = new Self({ title, profilePhoto, description, status });
    await newSelf.save();
...
export const updateSelf = async (request, reply) => {
  try {
    ...
    const updatedSelf = await Self.findByIdAndUpdate(
      id,
      { title, profilePhoto, description, status },
      { new: true },
    );
...
// DELETE entry by ID
export const deleteSelf = async (request, reply) => {
  try {
    const { id } = request.params;

    const data = await Self.findByIdAndDelete(id);
